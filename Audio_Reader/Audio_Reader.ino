...
int larger_low=0; // to see if  input was larger than before
int larger_mid=0; // to see if  input was larger than before
int larger_high=0; // to see if  input was larger than before
int holder=0;
/*
...
static const uint8_t PROGMEM
  ...
  eq[64]={
   50, 50,50,20,20,20,20, 20, 20, 20, 20, 20, 14,  8,  4,  2,
...
//  //Serial.print("start");
// for(int p=0;p<30;p++){
//    //Serial.print(String(spectrum[p])+ " ");
//    delay(1000);
//    if(p%30 ==0){
//      //Serial.println();
//    }
//  }
  ...
  for(x=0; x<8; x++) {
    ...
    nBins  = pgm_read_byte(&data[0]) + 2; //bins to merge
    binNum = pgm_read_byte(&data[1]); //which index to start
...
//
//  for(int i=0; i<8;i++){
//    Serial.println("Level: "+String(i)+" Peak: "+ String(peak[i]));
//    //Serial.println(String(peak[i]));
//  }
  // Every third frame, make the peak pixels drop by 1:
  if(++dotCount >= 7) {
   // Serial.println("Already In");
      holder = max(peak[1], peak [2]);
      holder=max(peak [0], holder);
      if( holder > larger_low){
        setAllBlue(holder);
        larger_low=holder;
      }else{
        larger_low = larger_low *0.90;
        blueFade();
      }
      holder = max (peak[3], peak[4]);
      holder=max(peak [5], holder);
      if( holder > larger_mid){
        setAllGreen(holder);
        larger_mid=holder;
      }else{
        larger_mid = larger_mid *0.90;
        greenFade();
      }
      holder=max(peak [6], peak[7]);
      if( holder > larger_high){
        setAllRed(holder);
        larger_high=holder;
      }else{
        larger_high = larger_high *0.90;
        redFade();
      }
      showStrip();
     // delay(1000);
...
void setAllBlue(byte blue) {
  for(int i = 0; i < NUM_LEDS; i++ ) {
    leds[i].b = blue;
  }
}
void setAllGreen(byte green) {
  for(int i = 0; i < NUM_LEDS; i++ ) {
    leds[i].g = green;
  }
}
void setAllRed(byte red) {
  for(int i = 0; i < NUM_LEDS; i++ ) {
    leds[i].r = red;
  }
}
...
void blueFade(){
    for(int i = 0; i < NUM_LEDS; i++ ) {
    leds[i].b=leds[i].b*0.90;
  }
}
void redFade(){
    for(int i = 0; i < NUM_LEDS; i++ ) {
    leds[i].r=leds[i].r*0.90;
  }
  //Serial.println(String(leds[0].r));
}
void greenFade(){
    for(int i = 0; i < NUM_LEDS; i++ ) {
    leds[i].g =leds[i].g* 0.90;
  }
}
void allFade(){
  for(int i = 0; i < NUM_LEDS; i++ ) {
    leds[i].fadeLightBy(25); 
  }
  showStrip();
}
